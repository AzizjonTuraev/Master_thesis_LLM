...
class REaLTabFormer:
    def __init__(
        self,
        model_type: str,
        generic_Config: Optional[PretrainedConfig] = GPT2Config,
        generic_LMHeadModel: Optional[PreTrainedModel] = GPT2LMHeadModel,
        my_config : Optional[dict] = {},
        tabular_config: Optional[PretrainedConfig] = None,
        ...
        **training_args_kwargs,
        ...
        Args:
            ...
            tabular_config: generic_Config instance to customize the GPT2 model for tabular data
                generation.
        ...
        self.generic_LMHeadModel = generic_LMHeadModel

        print(self.generic_config(**self.my_config))
    ...
    def _init_tabular(self, tabular_config):
        if tabular_config is not None:
            ...
            tabular_config = self.generic_config(**self.my_config)
    ...
    def _init_relational(self, relational_config):
        if relational_config is not None:
            ...
            relational_config = EncoderDecoderConfig(
                encoder=self.generic_config(**self.my_config).to_dict(),
                decoder=self.generic_config(**self.my_config).to_dict(),
            )
        ...
        if self.parent_gpt2_config is not None:
            ...
            relational_config.encoder = self.generic_config(**self.parent_gpt2_config)
    ...
    def _set_up_relational_coder_configs(self) -> None:
        def _get_coder(coder_name) -> PretrainedConfig:
        ...
        for coder_name in ["encoder", "decoder"]:
            ...
            if "n_positions" in coder.to_dict():
            ...
            elif "max_position_embeddings " in coder.to_dict():
            ...
            elif "max_positions " in coder.to_dict():


                if (
                    coder_name == "decoder"
                    and coder.max_positions  < self.relational_max_length
                ):
                    coder.max_positions  = 128 + self.relational_max_length
                elif coder_name == "encoder" and getattr(
                    coder, "max_positions ", getattr(coder, "max_position_embeddings")
                ) < len(self.vocab[coder_name]["column_token_ids"]):
                    positions = 128 + len(self.vocab[coder_name]["column_token_ids"])
                    coder.max_positions  = positions
                    try:
                        coder.max_positions  = positions
                    except:
                        coder.max_position_embeddings = positions
                ...
                raise ValueError("`n_positions` / `max_position_embedding` / 'max_positions' is defined.")
    ...
    def _fit_tabular(
        ...
        if "n_positions" in self.tabular_config.to_dict():
        ...
        elif "max_position_embeddings" in self.tabular_config.to_dict(): 

            if self.tabular_config.max_position_embeddings  < len(self.vocab["column_token_ids"]):
                self.tabular_config.max_position_embeddings  = 128 + len(self.vocab["column_token_ids"])

        elif "max_positions" in self.tabular_config.to_dict(): 

            if self.tabular_config.max_positions  < len(self.vocab["column_token_ids"]):
                self.tabular_config.max_positions  = 128 + len(self.vocab["column_token_ids"])

        else:
            raise ValueError("`n_positions` / `max_position_embedding` / 'max_positions' is defined.")

        self.model = self.generic_LMHeadModel(self.tabular_config)
    ...
    def save(self, path: Union[str, Path], allow_overwrite: Optional[bool] = False):
        ...
        # generic_Config is not JSON serializable, let us manually
        ...
        config_file.write_text(json.dumps(rtf_attrs, default=default_converter))
    ...
    @classmethod
    def load_from_dir(cls, path: Union[str, Path], instance):
        ...
        generic_config = instance.__dict__["generic_config"]
        generic_LMHeadModel = instance.__dict__["generic_LMHeadModel"]
        my_config = instance.__dict__["my_config"]
        ...
        try:
            realtf = cls(model_type=rtf_attrs["model_type"], 
                         ...
                         generic_LMHeadModel = generic_LMHeadModel, 
                         my_config = my_config)
        except KeyError:
        ...
        # special case for the generic_Config.
        for k, v in rtf_attrs.items():
            if k == "gpt_config":
                # Back-compatibility for saved models
                # before the support for relational data
                # was implemented.
                v = PretrainedConfig.from_dict(v)
                k = "tabular_config"
        ...
        realtf.generic_config = generic_config
        realtf.generic_LMHeadModel = generic_LMHeadModel
        realtf.my_config = my_config
        realtf.tabular_config = realtf.generic_config(**tabular_config)
        realtf.__dict__.pop("model")

        # Load model weights
        if realtf.model_type == ModelType.tabular:
            # realtf.model = PreTrainedModel(realtf.tabular_config)
            realtf.model = realtf.generic_LMHeadModel(realtf.tabular_config)
        elif realtf.model_type == ModelType.relational:
