...
from generic_be_great.great_dataset import GReaTDataset, GReaTDataCollator
from generic_be_great.great_start import (
    GReaTStart,
    CategoricalStart,
    ContinuousStart,
    RandomStart,
...
from generic_be_great.great_trainer import GReaTTrainer
from generic_be_great.great_utils import (
    _array_to_dataframe,
...
class GReaT:
    ...
    def __init__(
        self,
        llm: tp.Union[str, PreTrainedModel],
        ...
        **train_kwargs,
        ...
        print("This is a transformed GReaT model")
        ...
        if config:
            if my_config != {}:
                self.config = config(**my_config)
            else:
                self.config = config
        else:
            self.config = AutoModelForCausalLM.from_pretrained(self.llm).config
        ...
        if isinstance(self.llm, str):
            ...
            self.model = AutoModelForCausalLM.from_pretrained(self.llm, config=self.config)
        else:
            self.tokenizer = tokenizer
            self.tokenizer.pad_token = self.tokenizer.eos_token
            self.model = self.llm(self.config)
        ...
        print(self.config)
        self.pad_token_id = self.config.vocab_size - 1
...
#        ,device: str = "cuda",
            ...
            try:
                while n_samples > already_generated:
                    ...
                    tokens = self.model.generate(
                        ...
                        pad_token_id=self.pad_token_id,  # 50256
                    )
            ...
            while index < len(df_miss):
                ...
                while not is_complete:
                    ...
                    df_curr = self.great_sample(
                        starting_prompts, temperature, max_length, device=self.device
                    )
    ...
    def save(self, path: str):
        ...
        with open(path + "/config.json", "w") as f:
            ...
            json.dump(attributes, f, default=custom_serializer)
    ...
    def load_from_dir(cls, path: str, instance):
        """Load GReaT class
        ...
        llm = instance.__dict__["llm"]
        config = instance.__dict__["config"]
        tokenizer = instance.__dict__["tokenizer"]
        ...
        if isinstance(llm, str):
            great = cls(llm=llm, my_config = attributes["config"])
        else:
            # Create new be_great model instance
            great = cls(llm=llm, config=config, tokenizer=tokenizer)

        # Set all attributes
        attributes.pop("config")
        ...
        great.device = instance.device   #### To change the CUDA / CPU based on the local laptop capability
